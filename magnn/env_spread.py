...
class raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        scenario = Scenario()
        world = scenario.make_world(N, mask_prob=mask_prob)
        SimpleEnv.__init__(
            self,
            scenario=scenario,
            world=world,
...
class Scenario(BaseScenario):
    def make_world(self, N=3, mask_prob=0.0):
        world = World()
    ...
    def observation(self, agent, world):
        ...
        for entity in world.landmarks:  # world.entities:
            if np.random.uniform() < self.mask_prob:
                entity_pos.append(np.zeros(5))
            else:
                entity_pos.append(np.concatenate((
                    entity.state.p_vel,
        ...
        for other in world.agents:
            ...
            if np.random.uniform() < self.mask_prob:
                other_pos.append(np.zeros(5))
            else:
                other_pos.append(np.concatenate((
                    other.state.p_vel,
        ...
        distances = []
        for x in world.agents + world.landmarks:
            for y in world.agents + world.landmarks:
                if x is y:
                    continue
                distances.append(np.linalg.norm(x.state.p_pos - y.state.p_pos))
        return np.concatenate((
            agent.state.p_vel, agent.state.p_pos, [0.],  entity_pos , other_pos
        ))